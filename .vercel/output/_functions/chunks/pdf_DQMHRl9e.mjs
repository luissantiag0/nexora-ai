...
function drawPageFrame(page, fonts, pageNum, totalPages, doc) {
...
function drawTableHeader(page, y, cols, fonts) {
  page.drawRectangle({
    x: MARGIN,
    y: y - 22,
    width: CONTENT_W,
...
async function generatePitchDeckPdf(data) {
  const doc = await PDFDocument.create();
  const fonts = await getFonts(doc);
  const { regular, bold } = fonts;
...
async function generateEbookPdf(data) {
  const doc = await PDFDocument.create();
  const fonts = await getFonts(doc);
  const { regular, bold } = fonts;
  const cover = doc.addPage([PAGE_W, PAGE_H]);
...
export { generateInvoicePdf as a, generatePitchDeckPdf as b, generateReportPdf as c, generateEbookPdf as g };
