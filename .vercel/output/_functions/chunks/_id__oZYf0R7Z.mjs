...
import { getUserLead, LEAD_STATUSES } from './crm_CRjbA4A0.mjs';
...
const $$id = createComponent(async ($$result, $$props, $$slots) => {
  ...
  if (Astro2.request.method === "POST") {
    ...
    if (method === "PATCH") {
      ...
      const { updateUserLead } = await import('./crm_CRjbA4A0.mjs');
    ...
    if (method === "DELETE") {
      const { deleteUserLead } = await import('./crm_CRjbA4A0.mjs');
