...
export const GET: APIRoute = async ({ cookies, params, url }) => {
  ...
  if (url.searchParams.get("download") === "1") {
    const pdf = await generateEbookPdf({ title: ebook.title, content: ebook.content || "Sin contenido" });
    return new Response(pdf, {
      status: 200,
      headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="ebook-${ebook.id.slice(0, 8)}.pdf"` },
    });
  }
